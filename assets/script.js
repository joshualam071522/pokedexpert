...
for (let i = 0; i < storedPokemon.length; i++) {
  const nameOfPokemon = storedPokemon[i].parse.title;

  // <button class="button is-rounded is-small is-danger is-light">Pichu</button> 
  ...
  recentSearchesBtn.append(nameOfPokemon.slice(0,-10));
...
function getPokemonWikiDetails (event) {
  event.preventDefault()
//* fetching pageid from pokemon wiki
  var pokemonName = searchField.value;
    ...
    .then(function (data) {
      ...
      storedPokemon.push(pokemondata);
